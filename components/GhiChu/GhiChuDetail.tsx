...
// Port từ frontend/src/components/GhiChu/GhiChuDetail.jsx — modal xem chi tiết 1 ghi chú.
...
function deadlineStatus(dl?: string | null): "overdue" | "soon" | "ok" | null {
  if (!dl) return null;
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const d = new Date(dl.split("T")[0]);
  d.setHours(0, 0, 0, 0);
...
export default function GhiChuDetail({
  item,
  congTrinhName,
  onClose,
  onEdit,
  ...
  const dlBadge = dlStatus === "overdue" ? "bg-hp-danger/15 text-hp-danger" : dlStatus === "soon" ? "bg-hp-warning/15 text-hp-warning" : "bg-hp-surface text-hp-text-muted";
      ...
      <div className="bg-hp-elevated rounded-hp-xl shadow-md w-full max-w-lg">
        ...
        <div className="flex items-start justify-between px-6 pt-5 pb-3">
          <div className="flex-1 min-w-0 pr-4">
            {congTrinhName && <p className="text-xs text-hp-accent font-medium mb-1">{congTrinhName}</p>}
            <h2 className={`text-lg font-bold text-hp-text leading-snug ${isDone ? "line-through text-hp-text-muted" : ""}`}>{item.tieu_de}</h2>
          </div>
          <button onClick={onClose} className="p-1.5 hover:bg-hp-surface rounded-hp-sm text-hp-text-muted flex-shrink-0">
            <X className="w-5 h-5" />
          </button>
        </div>

        <div className="flex flex-wrap gap-2 px-6 pb-4">
          <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${trangThai.badge}`}>{trangThai.label}</span>
          <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${uuTien.badge}`}>
            {uuTien.icon} {uuTien.label}
          </span>
          <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${mau.badge}`}>{mau.label}</span>
        </div>

        <div className="px-6 pb-5 space-y-4">
          {item.noi_dung ? <div className={`p-4 rounded-hp-md text-sm text-hp-text whitespace-pre-wrap leading-relaxed ${mau.bg}`}>{item.noi_dung}</div> : <p className="text-sm text-hp-text-muted italic">Không có nội dung chi tiết.</p>}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-hp-border">
          <div>
            {!delConfirm ? (
              <button onClick={() => setDelConfirm(true)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-hp-text-muted hover:text-hp-danger hover:bg-hp-danger/10 rounded-hp-sm transition-colors">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-hp-danger text-white text-xs rounded-hp-sm font-medium"
                >
                  Xóa
                </button>
                <button onClick={() => setDelConfirm(false)} className="px-3 py-1.5 bg-hp-elevated text-hp-text-secondary text-xs rounded-hp-sm">
                  Không
                </button>
              </div>
            )}
