...
// Port từ frontend/src/pages/ct/CTAIReader.jsx.
import { useState, useRef } from "react";
import { Upload, FileText, Loader, CheckCircle, AlertCircle, Save, X, Plus, Trash2, RefreshCw, Zap, ChevronDown, ChevronUp } from "lucide-react";
import { docPhieu, docPhieuMulti, createPhieu } from "@/lib/api-client";
import { useCT } from "@/components/ct/CTProvider";
import type { PhieuData, PhieuItem } from "@/lib/ai/reader";
...
interface EditableItem extends PhieuItem {
  thanh_tien: number;
}

function toEditableItems(items: PhieuItem[]): EditableItem[] {
  return items.map((it) => ({
...
function PhieuEditor({
  ...
  const [doiTac, setDoiTac] = useState(data.doi_tac || "");
  const [items, setItems] = useState<EditableItem[]>(toEditableItems(data.items || []));
  const [saving, setSaving] = useState(false);
  const [saveMsg, setSaveMsg] = useState<{ type: "ok" | "err"; text: string } | null>(null);

  const updateItem = (i: number, field: keyof EditableItem, val: string) => {
    const next = [...items];
    const item = { ...next[i], [field]: field === "ten_hang" || field === "dvt" ? val : parseFloat(val) || 0 };
  ...
  const tongTien = items.reduce((s, it) => s + (it.thanh_tien || 0), 0);

  const handleSave = async () => {
    if (!soPhieu) {
      setSaveMsg({ type: "err", text: "Vui lòng nhập số phiếu" });
      return;
    }
    const validItems = items.filter((it) => it.ten_hang && it.so_luong > 0);
    if (validItems.length === 0) {
      setSaveMsg({ type: "err", text: "Cần ít nhất 1 dòng hàng" });
      return;
    }
    setSaving(true);
    setSaveMsg(null);
    ...
    <div className="space-y-4">
      ...
      <div className="border border-hp-border rounded-hp-lg overflow-x-auto">
            ...
            {items.map((it, i) => (
              <tr key={i} className="border-t border-hp-border">
                ...
                <td className="px-3 py-1.5">
                  <input
                    value={it.ten_hang}
                    onChange={(e) => updateItem(i, "ten_hang", e.target.value)}
                ...
                <td className="px-3 py-1.5 text-right text-xs font-medium text-hp-text">{formatVND(it.thanh_tien)}</td>
                <td className="px-2">
                  <button
        ...
        <div className="px-3 py-2 border-t border-hp-border flex justify-between items-center bg-hp-surface">
          <button
            onClick={() => setItems([...items, { ten_hang: "", dvt: "cai", so_luong: 0, don_gia: 0, thanh_tien: 0 }])}
      ...
      <div className="flex justify-end gap-3">
        {onCancel && (
          <button onClick={onCancel} className="min-h-10 px-5 py-2 border border-hp-border rounded-hp-md text-sm text-hp-text-secondary hover:bg-hp-elevated">
            Hủy
          </button>
        )}
        <button
...
// ── Danh sách nhiều phiếu (từ PDF nhiều phiếu) ────────────────
function MultiPhieuList({ phieuList, loai, ctId }: { phieuList: PhieuData[]; loai: "NK" | "XK"; ctId: string }) {
  const [expanded, setExpanded] = useState<number | null>(null);
  const [savedSet, setSavedSet] = useState<Set<number>>(new Set());
  const [savingAll, setSavingAll] = useState(false);
  const [saveAllMsg, setSaveAllMsg] = useState<{ type: "ok" | "err"; text: string } | null>(null);
  ...
  const handleSaveAll = async () => {
    ...
    for (let idx = 0; idx < phieuList.length; idx++) {
      ...
      if (!p.so_phieu) {
        errors.push(`Phiếu ${idx + 1}: thiếu số phiếu`);
        continue;
      }
      const validItems = (p.items || []).filter((it) => it.ten_hang && it.so_luong > 0);
      if (!validItems.length) {
        errors.push(`Phiếu ${idx + 1}: không có dòng hàng`);
        continue;
      }
      try {
        await createPhieu({
          ...
          ngay: p.ngay || today(),
          doi_tac: p.doi_tac || "",
          tong_tien: validItems.reduce((s, it) => s + (it.so_luong || 0) * (it.don_gia || 0), 0),
          items: validItems.map((it) => ({
            ten_hang: it.ten_hang || "",
            dvt: it.dvt || "cái",
            so_luong: it.so_luong || 0,
        ...
        errors.push(`${p.so_phieu}: ${errDetail(e, "Lỗi không xác định")}`);
      }
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-hp-success">
          ...
          <span className="font-semibold">
            AI đọc xong — tìm thấy <span className="font-bold">{phieuList.length} phiếu</span>
          </span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-xs text-hp-text-muted">
            {savedSet.size}/{phieuList.length} đã lưu
          </span>
          {savedSet.size < phieuList.length && (
            ...
            >
              {savingAll ? (
                <>
                  <Loader className="w-3.5 h-3.5 animate-spin" /> Đang lưu...
                </>
              ) : (
                <>
                  <Save className="w-3.5 h-3.5" /> Lưu tất cả ({phieuList.length - savedSet.size})
                </>
              )}
            </button>
          )}
      ...
      {phieuList.map((p, idx) => {
        ...
        return (
          <div key={idx} className={`border rounded-hp-lg overflow-hidden transition-colors ${isSaved ? "border-hp-success/40 bg-hp-success/5" : "border-hp-border bg-hp-card"}`}>
            <button className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-hp-elevated/50 transition-colors" onClick={() => setExpanded(isOpen ? null : idx)}>
...
// ── Trang chính ──────────────────────────────────────────────
export default function CTAIReaderPage() {
  const { ctId } = useCT();
  const [file, setFile] = useState<File | null>(null);
  const [loai, setLoai] = useState<"NK" | "XK">("NK");
  ...
  const [multiResult, setMultiResult] = useState<PhieuData[] | null>(null);
  ...
  const handleDoc = async () => {
    ...
    try {
      ...
      if (isPdf) {
        ...
        const list = ((res.data as { phieu_list?: PhieuData[] })?.phieu_list) || [];
        if (list.length === 1) {
          setResult(list[0]);
        } else {
          setMultiResult(list);
      ...
      setError(errDetail(e, "Có lỗi khi đọc phiếu. Thử lại sau."));
    } finally {
      setLoading(false);
    }
  };

  const providerLabel = provider === "gemini" ? "🆓 Gemini (Free)" : provider === "openai" ? "🤖 ChatGPT" : "Claude Sonnet";
  ...
  return (
    <div className="space-y-6">
      ...
      {result && (
        <div className="bg-hp-card rounded-hp-lg border border-hp-border p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-hp-accent/15 text-hp-accent text-xs font-medium rounded-full border border-hp-accent/30">{providerLabel}</span>
          </div>
          <PhieuEditor
            data={result}
            loai={loai}
      ...
      {multiResult && (
        <div className="bg-hp-card rounded-hp-lg border border-hp-border p-5">
          <MultiPhieuList phieuList={multiResult} loai={loai} ctId={ctId} />
        </div>
      )}
    </div>
  );
