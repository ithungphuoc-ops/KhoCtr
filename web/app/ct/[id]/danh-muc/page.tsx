...
// Port từ frontend/src/pages/ct/CTDanhMuc.jsx.
import { useState, useEffect } from "react";
import { Plus, Search, RefreshCw, Trash2, Package, X, Pencil } from "lucide-react";
import { getHangHoa, createHangHoa, deleteHangHoa, updateHangHoa } from "@/lib/api-client";
...
export default function CTDanhMucPage() {
  const { ctId } = useCT();
  ...
  const [editItem, setEditItem] = useState<HangHoa | null>(null);
  const [editForm, setEditForm] = useState({ ten_hang: "", dvt: "", nhom: "" });
  const [editSaving, setEditSaving] = useState(false);
  const [editMsg, setEditMsg] = useState<{ err: boolean; text: string } | null>(null);

  const loadData = () => {
    setLoading(true);
    getHangHoa({ cong_trinh_id: ctId, limit: 2000 })
      .then((res) => setList((res.data as { data?: HangHoa[] })?.data || []))
      .catch(() => {})
  ...
  const filtered = list.filter((h) => !search || (h.ten_hang || "").toLowerCase().includes(search.toLowerCase()) || (h.ma_hang || "").toLowerCase().includes(search.toLowerCase()));

  const handleSubmit = async () => {
    if (!form.ten_hang.trim()) {
      setMsg({ err: true, text: "Nhập tên hàng hóa" });
      return;
    }
    setSaving(true);
    setMsg(null);
    try {
      await createHangHoa({ ...form, cong_trinh_id: Number(ctId) });
      setMsg({ err: false, text: "Đã thêm: " + form.ten_hang });
      setForm({ ten_hang: "", dvt: "cái", nhom: "Vật tư", mo_ta: "" });
      loadData();
    } catch (e) {
      setMsg({ err: true, text: errDetail(e, "Lỗi thêm hàng hóa") });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (ma: string) => {
    ...
    try {
      ...
      alert("Lỗi xóa: " + errDetail(e, (e as Error).message));
    }
  };

  const openEdit = (h: HangHoa) => {
    setEditItem(h);
  ...
  const handleEditSave = async () => {
    if (!editForm.ten_hang.trim() || !editItem) {
      setEditMsg({ err: true, text: "Nhập tên hàng hóa" });
      return;
    }
    setEditSaving(true);
    setEditMsg(null);
    try {
      ...
      setEditMsg({ err: true, text: errDetail(e, "Lỗi cập nhật") });
    } finally {
      setEditSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="bg-hp-card rounded-hp-lg border border-hp-border p-5 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select value={form.nhom} onChange={(e) => setForm((f) => ({ ...f, nhom: e.target.value }))} className="w-full bg-hp-surface border border-hp-border rounded-hp-md px-3 py-2 text-sm text-hp-text focus:outline-none focus:ring-2 focus:ring-hp-accent min-h-10">
          ...
          {msg && <div className={`mt-3 text-sm px-3 py-2 rounded-hp-md ${msg.err ? "bg-hp-danger/10 text-hp-danger" : "bg-hp-success/15 text-hp-success"}`}>{msg.err ? "✗" : "✓"} {msg.text}</div>}

          <div className="flex gap-2 mt-4">
            <button onClick={() => setShowForm(false)} className="flex-1 py-2 border border-hp-border text-hp-text-secondary rounded-hp-md text-sm hover:bg-hp-elevated min-h-10">
              Hủy
            </button>
            <button onClick={handleSubmit} disabled={saving} className="flex-1 py-2 bg-hp-primary text-white rounded-hp-md text-sm font-medium hover:bg-hp-primary/90 disabled:opacity-50 flex items-center justify-center gap-2 min-h-10">
      ...
      <div className="bg-hp-card rounded-hp-lg border border-hp-border overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            {!search && (
              <button onClick={() => setShowForm(true)} className="mt-3 text-hp-accent text-sm hover:underline">
                  ...
                  <tr key={h.ma_hang || i} className="hover:bg-hp-elevated transition-colors">
                    ...
                    <td className="px-2 py-3">
                      <div className="flex gap-1">
                        <button onClick={() => openEdit(h)} className="p-1.5 text-hp-text-muted hover:text-hp-accent hover:bg-hp-accent/10 rounded-hp-md transition-colors">
                          <Pencil className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => handleDelete(h.ma_hang)} className="p-1.5 text-hp-text-muted hover:text-hp-danger hover:bg-hp-danger/10 rounded-hp-md transition-colors">
              ...
              {filtered.map((h, i) => (
                <CardListItem key={h.ma_hang || i}>
                  ...
                  <div className="flex items-center justify-end gap-1 pt-1 border-t border-hp-divider">
                    <button onClick={() => openEdit(h)} aria-label="Sửa hàng hóa" className="min-w-11 min-h-11 flex items-center justify-center text-hp-text-muted hover:text-hp-accent hover:bg-hp-accent/10 rounded-hp-md transition-colors">
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDelete(h.ma_hang)} aria-label="Xóa hàng hóa" className="min-w-11 min-h-11 flex items-center justify-center text-hp-text-muted hover:text-hp-danger hover:bg-hp-danger/10 rounded-hp-md transition-colors">
      ...
      {filtered.length > 0 && <p className="text-xs text-hp-text-muted text-center">{filtered.length} hàng hóa</p>}
          ...
          <div className="bg-hp-elevated rounded-hp-lg shadow-md w-full max-w-md">
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs text-hp-text-secondary mb-1 block">Nhóm</label>
                  <select value={editForm.nhom} onChange={(e) => setEditForm((f) => ({ ...f, nhom: e.target.value }))} className="w-full bg-hp-surface border border-hp-border rounded-hp-md px-3 py-2 text-sm text-hp-text focus:outline-none focus:ring-2 focus:ring-hp-accent min-h-10">
                    <option>Vật tư</option>
                    <option>Thiết bị</option>
                    <option>MM-CCDC</option>
                    <option>VPP</option>
              ...
              {editMsg && <div className={`text-sm px-3 py-2 rounded-hp-md ${editMsg.err ? "bg-hp-danger/10 text-hp-danger" : "bg-hp-success/15 text-hp-success"}`}>{editMsg.err ? "✗" : "✓"} {editMsg.text}</div>}

              <div className="flex gap-2 pt-1">
                <button onClick={() => setEditItem(null)} className="flex-1 py-2 border border-hp-border text-hp-text-secondary rounded-hp-md text-sm hover:bg-hp-surface min-h-10">
                  Hủy
                </button>
                <button onClick={handleEditSave} disabled={editSaving} className="flex-1 py-2 bg-hp-primary text-white rounded-hp-md text-sm font-medium hover:bg-hp-primary/90 disabled:opacity-50 flex items-center justify-center gap-2 min-h-10">
