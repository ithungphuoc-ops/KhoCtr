...
// Port từ frontend/src/pages/Dashboard.jsx (694 dòng) — GĐ2. Đổi: import ../api →
// @/lib/api-client (gọi Route Handler Next.js thay vì axios tới backend Python),
// context CongTrinh → CongTrinhProvider, theme tokens giữ nguyên. JSX/logic giữ
// nguyên gần như 100% để tối thiểu rủi ro sai lệch hành vi khi port.
...
import { HP } from "@/lib/theme-tokens";

interface KpiData {
  so_cong_trinh?: number;
  so_phieu_nk?: number;
  so_phieu_xk?: number;
  so_mat_hang?: number;
  so_canh_bao?: number;
  so_canh_bao_thap?: number;
  tong_tien_nk?: number;
  tong_tien_xk?: number;
  so_am_kho?: number;
}

interface TopVatTuItem {
  ten_hang: string;
  so_luong: number;
  thanh_tien: number;
  dvt?: string;
...
interface BieuDoPoint {
  period: string;
  tong_nk: number;
  tong_xk: number;
}

// ── Ghi chú widget (localStorage, độc lập với ghi_chu Firestore — port nguyên bản) ──
const NOTE_COLORS = [
...
const colorMap = Object.fromEntries(NOTE_COLORS.map((c) => [c.key, c])) as Record<string, (typeof NOTE_COLORS)[number]>;
...
function GhiChuWidget() {
  const [notes, setNotes] = useState<LocalNote[]>(() => {
    try {
      return JSON.parse(localStorage.getItem("khounice_ghi_chu") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const dlStatus = (dl: string | null): "overdue" | "soon" | "ok" | null => {
    if (!dl) return null;
    const d = new Date(dl);
    d.setHours(0, 0, 0, 0);
  ...
  const fmtDeadline = (dl: string | null) => {
    if (!dl) return null;
  ...
  const dlBorderMap: Record<string, string> = { overdue: "border-hp-danger", soon: "border-hp-warning", ok: "border-transparent" };
  const dlBadgeMap: Record<string, string> = {
    overdue: "bg-hp-danger/15 text-hp-danger",
    soon: "bg-hp-warning/15 text-hp-warning",
    ok: "bg-hp-success/15 text-hp-success",
  };
  const dlLabelMap: Record<string, string> = { overdue: "Quá hạn", soon: "Sắp đến", ok: "" };

  const addNote = () => {
    if (!draft.trim()) return;
    save([
      { id: Date.now(), noi_dung: draft.trim(), mau: color, deadline: deadline || null, created_at: new Date().toLocaleString("vi-VN") },
      ...notes,
    ]);
    setDraft("");
    setDeadline("");
  ...
  const startEdit = (n: LocalNote) => {
    setEditId(n.id);
    setEditText(n.noi_dung);
    ...
    <div className="bg-hp-card rounded-hp-lg shadow-sm border border-hp-border p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        {soonCount > 0 && overdueCount === 0 && (
          <span className="px-1.5 py-0.5 bg-hp-warning/15 text-hp-warning text-xs font-medium rounded-full">{soonCount} sắp đến hạn</span>
        )}
      ...
      <div className="flex gap-2 mb-4">
        <div className="flex-1 space-y-1.5">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            {deadline && (
              <button onClick={() => setDeadline("")} className="text-hp-text-muted hover:text-hp-text-secondary">
                <X className="w-3 h-3" />
              </button>
            )}
          </div>
                ...
                {editId === n.id ? (
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      />
                      {editDeadline && (
                        <button onClick={() => setEditDeadline("")} className="text-hp-text-muted hover:text-hp-text-secondary">
                          <X className="w-3 h-3" />
                        </button>
                      )}
                    </div>
                    <div className="flex gap-2 justify-end">
                      <button onClick={() => setEditId(null)} className="text-xs text-hp-text-muted hover:text-hp-text-secondary">
                        Hủy
                      </button>
                      <button onClick={() => saveEdit(n.id)} className="text-xs text-hp-accent font-medium hover:text-hp-accent/80">
                        Lưu
                      </button>
                    </div>
                  </div>
                  ...
                  <>
                    <p className="text-hp-text whitespace-pre-wrap cursor-pointer leading-relaxed" onClick={() => startEdit(n)}>
                      {n.noi_dung}
                    </p>
                    <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                      <p className="text-xs text-hp-text-muted">{n.created_at}</p>
                      {n.deadline && (
                        <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${dls ? dlBadgeMap[dls] : "bg-hp-muted/20 text-hp-text-secondary"}`}>
                          {dls ? dlLabelMap[dls] : ""} {fmtDeadline(n.deadline)}
                        </span>
                      )}
                    </div>
                    <button
...
function TonKhoWidget({ data, loading }: { data: TonKhoItem[]; loading: boolean }) {
  const [search, setSearch] = useState("");
  const filtered = data.filter((r) => !search || (r.ten_hang || "").toLowerCase().includes(search.toLowerCase()));
  return (
    <div className="bg-hp-card rounded-hp-lg shadow-sm border border-hp-border p-5">
                ...
                {filtered.map((r, i) => {
                  ...
                  return (
                    <tr key={i} className="hover:bg-hp-elevated">
                      <td className="py-1.5 text-hp-text max-w-[160px] truncate" title={r.ten_hang}>
                        {r.ten_hang}
                      </td>
...
function TopVatTuWidget({ title, data, loading, type }: { title: string; data: TopVatTuItem[]; loading: boolean; type: "nk" | "xk" }) {
      ...
      {loading ? (
        ...
        <div className="space-y-1.5 max-h-52 overflow-y-auto">
          {data.slice(0, 8).map((item, i) => {
            ...
            const label = item.thanh_tien > 0 ? formatVND(item.thanh_tien) : `${fmt(item.so_luong)} ${item.dvt || ""}`;
            return (
              <div key={i} className="flex items-center gap-2">
                <span className="text-xs text-hp-text-muted w-4 flex-shrink-0">{i + 1}</span>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-0.5">
                    <span className="text-xs text-hp-text truncate max-w-[160px]" title={item.ten_hang}>
                      {item.ten_hang}
                    </span>
...
export default function DashboardPage() {
  ...
  const [chartMode, setChartMode] = useState<"day" | "week" | "month" | "year">("month");
  const [loading, setLoading] = useState(true);
  const [kpi, setKpi] = useState<KpiData | null>(null);
  const [topNK, setTopNK] = useState<TopVatTuItem[]>([]);
  const [topXK, setTopXK] = useState<TopVatTuItem[]>([]);
  const [bangCT, setBangCT] = useState<BangCtItem[]>([]);
  const [canhBao, setCanhBao] = useState<CanhBaoItem[]>([]);
  const [bieuDoData, setBieuDoData] = useState<BieuDoPoint[]>([]);
  ...
  const loadData = React.useCallback(() => {
    ...
    Promise.all([
      api.getBaoCaoTongHop({ ...ctParam, ...dateParam }),
      api.getBieuDo({ period: chartMode, from_date: dateFrom, to_date: dateTo, ...ctParam }),
    ])
      .then(([bcRes, bdRes]) => {
        const bc = (bcRes.data || {}) as Record<string, unknown>;
        setKpi((bc.kpi as KpiData) || {});
        setTopNK((bc.top_vat_tu_nk as TopVatTuItem[]) || []);
        setTopXK((bc.top_vat_tu_xk as TopVatTuItem[]) || []);
        setBangCT((bc.bang_cong_trinh as BangCtItem[]) || []);
        setCanhBao((bc.canh_bao_ton_thap as CanhBaoItem[]) || []);
        setBieuDoData(((bdRes.data as { data?: BieuDoPoint[] })?.data) || []);
        setTonKho((bc.ton_kho as TonKhoItem[]) || []);
      })
      .catch((err) => console.error("[Dashboard] Load data error:", err))
      .finally(() => setLoading(false));
  }, [chartMode, effectiveCTId, dateFrom, dateTo]);
  ...
  const barChartData = {
    ...
    datasets: [
      { type: "bar" as const, label: "Nhập", data: bieuDoData.map((d) => d.tong_nk), backgroundColor: HP.accent, borderRadius: 4, yAxisID: "y" },
      { type: "bar" as const, label: "Xuất", data: bieuDoData.map((d) => d.tong_xk), backgroundColor: HP.warning, borderRadius: 4, yAxisID: "y" },
    ],
  };
  ...
  const barChartOptions = {
    ...
    plugins: { legend: { position: "top" as const, labels: { font: { size: 11 }, padding: 10, color: HP.textSecondary } } },
    scales: {
      x: { grid: { color: HP.border }, ticks: { color: HP.textSecondary } },
      y: { type: "linear" as const, display: true, position: "left" as const, grid: { color: HP.border }, ticks: { color: HP.textSecondary }, beginAtZero: true },
    },
  };

  const donutData = {
    labels: ["Nhập kho", "Xuất kho"],
    datasets: [{ data: [phieuNhap || 1, phieuXuat || 1], backgroundColor: [HP.primary, HP.warning], borderColor: [HP.primary, HP.warning], borderWidth: 2, hoverOffset: 4 }],
  };
  ...
  const donutOptions = {
    ...
    maintainAspectRatio: false,
    cutout: "65%",
    plugins: { legend: { position: "bottom" as const, labels: { font: { size: 11 }, padding: 12, color: HP.textSecondary } } },
  };

  const tongCT = bangCT.reduce(
    (a, c) => ({
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "5fr 3fr" }}>
        <div className="bg-hp-card rounded-hp-lg shadow-sm border border-hp-border p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <select
              value={chartMode}
              onChange={(e) => setChartMode(e.target.value as typeof chartMode)}
          ...
          <div style={{ height: 220 }}>
            {loading ? <div className="h-full flex items-center justify-center text-hp-text-muted text-sm">Đang tải...</div> : <Bar data={barChartData} options={barChartOptions} />}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-hp-card rounded-hp-lg shadow-sm border border-hp-border p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="bg-hp-danger/15 text-hp-danger text-xs font-semibold px-2 py-0.5 rounded-full">{canhBao.length} cảnh báo</span>
          </div>
          {loading ? (
            <div className="text-hp-text-muted text-sm text-center py-4">Đang tải...</div>
          ) : canhBao.length === 0 ? (
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {bangCT.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="py-4 text-center text-hp-text-muted">
                        Chưa có dữ liệu
                      </td>
                    </tr>
                  ) : (
                    bangCT.map((ct, i) => (
                      <tr key={ct.id || i} className="border-b border-hp-divider hover:bg-hp-elevated">
                        ...
                        <td className="py-2 text-hp-text truncate max-w-[160px]" title={ct.ten_ct}>
                          {ct.ten_ct}
                        </td>
                  ...
                  {bangCT.length > 0 && (
                    <tr className="border-t-2 border-hp-border font-bold bg-hp-surface">
                      <td className="py-2 text-hp-text-secondary" colSpan={2}>
                        Tổng cộng
                      </td>
            ...
            <div className="md:hidden space-y-2">
              {bangCT.length === 0 ? (
                ...
                bangCT.map((ct, i) => (
                  <div key={ct.id || i} className="bg-hp-surface rounded-hp-md p-2.5 text-sm">
                    ...
                    <div className="flex justify-between text-xs">
                      <span className="text-hp-primary">
                        NK: {ct.so_phieu_nk || 0} phiếu · {formatVND(ct.tong_tien_nk)}
                      </span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span className="text-hp-warning">
                        XK: {ct.so_phieu_xk || 0} phiếu · {formatVND(ct.tong_tien_xk)}
                      </span>
                    </div>
                  </div>
                ))
              ...
              {bangCT.length > 0 && (
                <div className="bg-hp-surface rounded-hp-md p-2.5 text-sm border-t-2 border-hp-border">
                  ...
                  <div className="flex justify-between text-xs">
                    <span className="text-hp-primary">
                      NK: {tongCT.so_phieu_nk} · {formatVND(tongCT.tong_tien_nk)}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-hp-warning">
                      XK: {tongCT.so_phieu_xk} · {formatVND(tongCT.tong_tien_xk)}
                    </span>
                  </div>
                </div>
              )}
      ...
      <div className="bg-hp-card rounded-hp-lg shadow-sm border border-hp-border p-5">
        ...
        <div className="flex items-stretch gap-2">
          {[
            ...
            { icon: RefreshCw, color: "bg-hp-primary", label: "ĐỒNG BỘ", items: ["Auto sync Firestore", "Real-time update", "Delta sync"] },
            { icon: Database, color: "bg-hp-nav", label: "DATABASE TỔNG", items: ["Firebase Firestore", "Real-time", "Backup tự động"] },
