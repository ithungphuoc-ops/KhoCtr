...
// Port từ frontend/src/pages/LichSuGiaoDich.jsx.
import { useState, useEffect } from "react";
import { Search, RefreshCw, Download, Upload } from "lucide-react";
import { getLichSuGiaoDich } from "@/lib/api-client";
...
export default function LichSuGiaoDichPage() {
  const { selectedCT, congTrinhs, dateFrom, dateTo, isAdmin } = useCongTrinh();
  const effectiveCT = isAdmin ? selectedCT : congTrinhs?.[0] || null;

  const [data, setData] = useState<LichSuRow[]>([]);
  ...
  const loadData = () => {
    ...
    getLichSuGiaoDich(params)
      .then((res) => {
        const d = res.data as { data?: LichSuRow[]; total?: number };
        setData(d?.data || []);
        setTotal(d?.total || 0);
      })
  ...
  }, [effectiveCT, loaiFilter, dateFrom, dateTo]);

  const filtered = data.filter((r) => {
    if (!search) return true;
    const q = search.toLowerCase();
    return (r.ten_hang || "").toLowerCase().includes(q) || (r.so_phieu || "").toLowerCase().includes(q) || (r.doi_tac || "").toLowerCase().includes(q);
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-hp-card rounded-hp-lg border border-hp-border p-4 flex items-center gap-3 cursor-pointer hover:border-hp-accent/50" onClick={() => setLoaiFilter(loaiFilter === "NK" ? "" : "NK")}>
        ...
        <div className="bg-hp-card rounded-hp-lg border border-hp-border p-4 flex items-center gap-3 cursor-pointer hover:border-hp-warning/50" onClick={() => setLoaiFilter(loaiFilter === "XK" ? "" : "XK")}>
      ...
      <div className="bg-hp-card rounded-hp-lg shadow-sm border border-hp-border p-4 flex gap-3 flex-wrap items-center">
        ...
        <div className="flex gap-1">
          {(["", "NK", "XK"] as const).map((v) => (
            <button
              key={v}
              onClick={() => setLoaiFilter(v)}
              className={`px-3 py-1.5 rounded-hp-md text-xs font-medium transition-colors ${
                loaiFilter === v ? (v === "NK" ? "bg-hp-accent text-white" : v === "XK" ? "bg-hp-warning text-white" : "bg-hp-nav text-white") : "bg-hp-elevated text-hp-text-secondary hover:text-hp-text"
              }`}
            >
              ...
              {loading ? (
                ...
                filtered.map((r, i) => (
                  <tr key={i} className="border-b border-hp-divider hover:bg-hp-elevated transition-colors">
                    ...
                    <td className="px-4 py-2.5 text-hp-text font-medium max-w-[280px] truncate">{r.ten_hang}</td>
                    ...
                    <td className={`px-3 py-2.5 text-right font-semibold text-xs ${r.loai === "NK" ? "text-hp-accent" : "text-hp-warning"}`}>{formatVND(r.thanh_tien)}</td>
                    <td className="px-3 py-2.5 text-center">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${r.loai === "NK" ? "bg-hp-accent/15 text-hp-accent" : "bg-hp-warning/15 text-hp-warning"}`}>{r.loai === "NK" ? "↓ NK" : "↑ XK"}</span>
                    </td>
          ...
          {filtered.map((r, i) => (
            <CardListItem key={i}>
              <div className="flex items-start justify-between gap-2">
                ...
                <span className={`flex-shrink-0 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${r.loai === "NK" ? "bg-hp-accent/15 text-hp-accent" : "bg-hp-warning/15 text-hp-warning"}`}>{r.loai === "NK" ? "↓ NK" : "↑ XK"}</span>
              </div>
