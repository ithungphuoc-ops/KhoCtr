...
// Port từ frontend/src/pages/CongTrinh.jsx.
import { useState, useEffect } from "react";
import { Building2, Search, RefreshCw, MapPin, Hash, FileText, Plus, Trash2, X, CheckCircle, XCircle, CheckCircle2, RotateCcw, AlertTriangle } from "lucide-react";
import { api, updateCongTrinhStatus } from "@/lib/api-client";
...
interface DeleteModalState {
  ...
  ct: CongTrinh | null;
  loading: boolean;
  counts: { phieu_count: number; hang_hoa_count: number; chi_tiet_count: number } | null;
}
...
export default function CongTrinhPage() {
  ...
  const [data, setData] = useState<CongTrinh[]>([]);
  ...
  const [deleteModal, setDeleteModal] = useState<DeleteModalState>({ show: false, ct: null, loading: false, counts: null });
  const [form, setForm] = useState({ ten_ct: "", ma_ct: "", dia_chi: "", ghi_chu: "" });

  const loadData = () => {
    setLoading(true);
    api
      .get<{ data: CongTrinh[] }>("/cong-trinh/")
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (!form.ten_ct || !form.ma_ct) {
      setMsg({ type: "err", text: "Vui lòng nhập Tên và Mã công trình" });
      return;
    }
    setSaving(true);
    setMsg(null);
    try {
      ...
      setMsg({ type: "err", text: errDetail(err, "Lỗi tạo công trình") });
    } finally {
      setSaving(false);
    }
  };

  const handleDeleteClick = async (ct: CongTrinh) => {
    setDeleteModal({ show: true, ct, loading: true, counts: null });
    try {
      const res = await api.get<DeleteModalState["counts"]>(`/cong-trinh/${ct.id}/stats`);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setMsg({ type: "err", text: errDetail(err, "Lỗi xóa công trình") });
      setDeleteModal((prev) => ({ ...prev, loading: false }));
    }
  };

  const handleToggleStatus = async (ct: CongTrinh) => {
    ...
    try {
      ...
      setMsg({ type: "err", text: errDetail(err, "Lỗi cập nhật trạng thái") });
    } finally {
      setUpdating(null);
    }
  };

  const getTrangThaiInfo = (trang_thai?: string) => {
  ...
  const dangHoatDong = data.filter((c) => !c.trang_thai || c.trang_thai === "hoat_dong").length;
  const hoanThanh = data.filter((c) => c.trang_thai === "hoan_thanh").length;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-hp-text-secondary mt-1 text-sm">
            {data.length} công trình — <span className="text-hp-primary">{dangHoatDong} hoạt động</span>
            {hoanThanh > 0 && <span className="text-hp-text-muted"> · {hoanThanh} hoàn thành</span>}
          </p>
        </div>
        <div className="flex gap-2">
          <button onClick={loadData} disabled={loading} className="p-2 border border-hp-border rounded-hp-lg hover:bg-hp-elevated text-hp-text-secondary">
      ...
      {msg && (
        <div
          className={`flex items-center gap-3 p-4 rounded-hp-xl border text-sm ${
            msg.type === "ok" ? "bg-hp-success/10 border-hp-success/30 text-hp-success" : "bg-hp-danger/10 border-hp-danger/30 text-hp-danger"
          }`}
        >
      ...
      {loading ? (
        ...
        <div className="text-center text-hp-text-muted py-12">
          ...
          <div className="font-medium">{search ? "Không tìm thấy kết quả" : "Chưa có công trình nào"}</div>
        ...
        <div className="grid grid-cols-1 gap-3">
          {filtered.map((ct, i) => {
            ...
            return (
              <div key={ct.id} className={`bg-hp-card rounded-hp-xl border p-5 flex items-start gap-4 hover:shadow-sm transition-all ${isHoanThanh ? "border-hp-border opacity-70" : "border-hp-border"}`}>
                <div className={`w-10 h-10 rounded-hp-xl flex items-center justify-center flex-shrink-0 font-bold text-sm ${isHoanThanh ? "bg-hp-elevated text-hp-text-muted" : "bg-hp-primary/20 text-hp-primary"}`}>
                  {i + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className={`font-bold ${isHoanThanh ? "text-hp-text-muted line-through" : "text-hp-text"}`}>{ct.ten_ct}</div>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${ttInfo.className}`}>{ttInfo.label}</span>
                      <button
                        onClick={() => handleToggleStatus(ct)}
                        disabled={isUpdating}
                        title={isHoanThanh ? "Hoạt động lại" : "Đánh dấu Hoàn thành"}
                        className={`p-1.5 rounded-hp-lg transition-colors disabled:opacity-50 ${isHoanThanh ? "text-hp-text-muted hover:text-hp-primary hover:bg-hp-primary/10" : "text-hp-text-muted hover:text-hp-warning hover:bg-hp-warning/10"}`}
                      >
                        {isUpdating ? <RefreshCw className="w-4 h-4 animate-spin" /> : isHoanThanh ? <RotateCcw className="w-4 h-4" /> : <CheckCircle2 className="w-4 h-4" />}
                      </button>
                      <button onClick={() => handleDeleteClick(ct)} title="Xóa công trình" className="p-1.5 text-hp-text-muted hover:text-hp-danger hover:bg-hp-danger/10 rounded-hp-lg transition-colors">
                  ...
                  {ct.ghi_chu && (
                    <div className="flex items-center gap-1.5 mt-1">
                      ...
                      <span className="text-xs text-hp-text-muted italic">{String(ct.ghi_chu)}</span>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="bg-hp-elevated rounded-hp-xl shadow-md w-full max-w-md p-6">
            <div className="flex items-start gap-4 mb-5">
              ...
              <div>
                ...
                <p className="text-sm text-hp-text-secondary mt-0.5 font-mono">
                  {deleteModal.ct?.ma_ct} — {deleteModal.ct?.ten_ct}
                </p>
              </div>
            </div>

            <div className="bg-hp-danger/10 border border-hp-danger/30 rounded-hp-xl p-4 mb-4">
              <p className="text-sm font-semibold text-hp-danger">Bạn có chắc chắn muốn xóa công trình này?</p>
              <p className="text-sm text-hp-danger mt-1">Toàn bộ dữ liệu sẽ bị xóa vĩnh viễn và không thể khôi phục.</p>
            </div>

            {deleteModal.loading && !deleteModal.counts && <p className="text-sm text-hp-text-muted text-center py-3">Đang kiểm tra dữ liệu...</p>}
            ...
            <div className="flex gap-3 mt-2">
              ...
              <button onClick={handleDeleteConfirm} disabled={deleteModal.loading} className="flex-1 py-2.5 bg-hp-danger text-white rounded-hp-xl text-sm font-semibold hover:bg-hp-danger/90 disabled:opacity-50 transition-colors">
          ...
          <div className="bg-hp-elevated rounded-hp-xl shadow-md w-full max-w-md p-6">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                ...
                <input
                  value={form.ten_ct}
                  onChange={(e) => setForm({ ...form, ten_ct: e.target.value })}
              ...
              <div>
                ...
                <input
                  value={form.ma_ct}
                  onChange={(e) => setForm({ ...form, ma_ct: e.target.value.toUpperCase() })}
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowForm(false)} className="flex-1 py-2.5 border border-hp-border text-hp-text-secondary rounded-hp-xl text-sm hover:bg-hp-elevated">
                  Hủy
                </button>
                <button type="submit" disabled={saving} className="flex-1 py-2.5 bg-hp-primary text-white rounded-hp-xl text-sm font-medium hover:bg-hp-primary/90 disabled:opacity-50">
