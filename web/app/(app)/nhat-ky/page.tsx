...
// Port từ frontend/src/pages/NhatKy.jsx.
import { useState, useEffect } from "react";
import { ClipboardList, RefreshCw, Search, Filter as FilterIcon } from "lucide-react";
import { getNhatKy } from "@/lib/api-client";
...
import type { ActivityLogEntry } from "@/lib/data/nhat-ky";
...
export default function NhatKyPage() {
  ...
  const loadData = () => {
    ...
    getNhatKy(params)
      .then((res) => setLogs((res.data as { data?: ActivityLogEntry[] })?.data || []))
  ...
  }, [page, filterAction, selectedCT]);

  const filtered = logs.filter(
    (l) => !search || (l.user_email || "").toLowerCase().includes(search.toLowerCase()) || (l.details || "").toLowerCase().includes(search.toLowerCase()) || (l.action || "").toLowerCase().includes(search.toLowerCase()),
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={loadData} disabled={loading} className="flex items-center gap-2 px-4 py-2 bg-hp-accent/15 hover:bg-hp-accent/25 text-hp-accent rounded-hp-md text-sm font-medium disabled:opacity-50">
      ...
      <div className="bg-hp-card rounded-hp-lg border border-hp-border p-4 flex gap-3 flex-wrap items-center">
        ...
        <div className="flex items-center gap-2">
          <FilterIcon className="w-4 h-4 text-hp-text-muted" />
          <select
            value={filterAction}
            onChange={(e) => {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={6} className="py-10 text-center text-hp-text-muted">
                    Đang tải...
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                ...
                filtered.map((log, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-hp-border hover:bg-hp-elevated transition-colors">
                      ...
                      <td className="px-4 py-3 text-hp-text-secondary text-xs max-w-[280px] truncate" title={log.details}>
                        {log.details || "—"}
                      </td>
                      <td className="px-4 py-3 text-hp-text-secondary text-xs truncate max-w-[120px]">{log.cong_trinh_id ? ctMap[log.cong_trinh_id] || `CT #${log.cong_trinh_id}` : "—"}</td>
                    </tr>
                  );
                })
              )}
      ...
      <div className="flex items-center justify-between">
        <button onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0 || loading} className="px-4 py-2 border border-hp-border rounded-hp-md text-sm text-hp-text-secondary hover:bg-hp-elevated disabled:opacity-40">
          ← Trang trước
        </button>
        <span className="text-sm text-hp-text-secondary">Trang {page + 1}</span>
        <button onClick={() => setPage((p) => p + 1)} disabled={logs.length < PAGE_SIZE || loading} className="px-4 py-2 border border-hp-border rounded-hp-md text-sm text-hp-text-secondary hover:bg-hp-elevated disabled:opacity-40">
          Trang sau →
        </button>
      </div>
    </div>
