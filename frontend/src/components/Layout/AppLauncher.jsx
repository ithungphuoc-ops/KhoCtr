// AppLauncher — danh sách ứng dụng công ty (giống hệt PKD/app tổng), lấy từ
// account.hpcore.vn/api/apps. Bấm vào logo/tên app ở đầu Sidebar để mở.
...
const ROLE_LABEL = { admin: 'Quản trị viên', user: 'Thủ kho' }
const isBiz = (n) => n.startsWith('HPC ')

function Tile({ app, onNavigate }) {
  const Icon = (app.iconKey && ICONS[app.iconKey]) || AppWindow
  const current = !!app.href && app.href.includes('khoct.hpcore.vn')
  const inner = (
    <>
      <div className={`flex size-14 items-center justify-center overflow-hidden rounded-hp-xl transition-transform group-hover:scale-105
      ...
      <span className={`text-center text-xs font-medium leading-tight ${app.comingSoon ? 'text-hp-text-disabled' : 'text-hp-text'}`}>
        {app.name}
      </span>
      {current && <span className="rounded-full bg-hp-primary/15 px-1.5 py-0.5 text-[9px] text-hp-primary">Đang dùng</span>}
      {app.comingSoon && <span className="rounded-full bg-hp-warning/15 px-1.5 py-0.5 text-[9px] text-hp-warning">Sắp ra mắt</span>}
    </>
  )
...
export default function AppLauncher({ user, onClose }) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-start justify-start overflow-y-auto bg-hp-overlay p-3 sm:py-4 sm:pl-[calc(var(--hp-sidebar-width,260px)+12px)] sm:pr-4"
      onClick={onClose}
    >
      <div className="w-full max-w-4xl rounded-hp-xl border border-hp-border bg-hp-surface text-hp-text shadow-2xl" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="flex flex-col gap-3 border-b border-hp-border p-5 sm:flex-row sm:items-center">
          <div className="min-w-0">
            ...
            <p className="text-xs text-hp-text-muted">
              ...
              <a href={HPCORE_PROFILE_URL} target="_blank" rel="noopener noreferrer" className="text-hp-accent hover:underline">Tài khoản</a>
            </p>
          </div>
          ...
          <button onClick={onClose} aria-label="Đóng" className="hidden size-8 items-center justify-center rounded-hp-md text-hp-text-muted hover:bg-white/8 sm:flex">
            <X className="size-5" />
          </button>
        </div>
        ...
        <div className="max-h-[70vh] space-y-6 overflow-y-auto p-5">
          <a href="https://account.hpcore.vn/dashboard" className="inline-flex items-center gap-2 text-sm text-hp-accent hover:underline">
