...
- [x] 4.4 Port `ai_routes.py` (412 dòng, phần doc-phieu/doc-phieu-multi) — `lib/ai/reader.ts` + `lib/ai/resolve-key.ts` + Route Handler `/api/ai/doc-phieu`, `/api/ai/doc-phieu-multi` + trang `ai-reader` (App Tổng) và `ct/[id]/ai-reader` (App Con). **Đơn giản hoá có chủ đích**: bản gốc dùng PyMuPDF (fitz) render PDF→PNG trước khi gửi Claude/OpenAI (đọc chữ viết tay tốt hơn) + Anthropic Files API batch cho PDF nhiều trang — Node không có fitz tương đương, viết renderer riêng (pdfjs-dist+canvas) tốn nhiều công nên **chưa làm**. Bản port gửi thẳng file gốc (base64) cho Gemini (inlineData, khớp bản gốc y hệt) và Claude (content block "document", khớp đúng nhánh fallback-không-fitz đã có sẵn trong code gốc) — quality đọc chữ viết tay có thể kém hơn PNG nhưng vẫn hoạt động cho phiếu in/scan rõ. OpenAI Chat Completions không nhận PDF trực tiếp — tạm chặn PDF cho provider này với thông báo rõ ràng (dùng Gemini/Claude cho PDF). Thêm `requireSession()` cho 2 route này (bản gốc không có auth check nào).
- [x] 4.4b Port `mapping_service.py` (315 dòng, fuzzy match Jaccard Token) + `fuzzy_match.py` (182 dòng) sang `lib/ai/fuzzy-match.ts` + `lib/data/ai-mapping.ts` — verify khớp 100% với bản Python gốc (normalize + fuzzy_score, nhiều test case tiếng Việt có dấu). Route Handler `/api/ai/match-items`, `/api/ai/confirm-match`. Port `components/BatchPhieuPopup.tsx` (331 dòng) + nối vào `ct/[id]/nhap-kho`, `ct/[id]/xuat-kho` — thay nút "AI đọc PDF" (trước đây là `alert()` placeholder) bằng luồng thật: chọn provider → upload → `docPhieu`/`docPhieuMulti` → `matchItems` phân loại 🟢🟡🔴 → `BatchPhieuPopup` xác nhận/sửa → lưu hàng loạt qua `createPhieu`.
